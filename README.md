...
- Scripts and stylesheets are linked with a version tag (for example `site.js?v=2026-09-24i`) so browsers pick up changes at once instead of keeping an old copy for about ten minutes. After editing a file in `assets/css` or `assets/js`, change that tag in the HTML pages (one find-and-replace).
